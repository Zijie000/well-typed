...
let classify_word w =
  ...
  | "forall" -> ForAll
  | "lambda" -> Lambda
  | "tlam" | "lambdat" | "biglambda" | "blambda" -> TypeLambda
  | "top" -> TyTop
  | "bool" -> TyBool
  | "nat" -> TyNat
  | _ -> Ident w
...
let lex input =
  ...
  let rec aux i acc =
    if i >= len then List.rev (EOF :: acc)
      ...
      | '.' -> aux (i + 1) (Dot :: acc)
      | '\\' -> aux (i + 1) (Lambda :: acc)
      | '<' when i + 1 < len && input.[i + 1] = ':' -> aux (i + 2) (Subtype :: acc)
...
let rec parse_type tokens =
  match tokens with
  | ForAll :: Ident v :: Subtype :: rest ->
      let bound, rest1 = parse_type rest in
      begin
        match rest1 with
        | Dot :: rest2 ->
            let body, rest3 = parse_type rest2 in
            (TyForall (v, bound, body), rest3)
        | _ -> raise (Parse_error "Expected '.' in universal type")
      end
...
and parse_type_lambda = function
  | TypeLambda :: Ident param :: Subtype :: rest ->
      let bound, rest1 = parse_type rest in
      begin
        match rest1 with
        | Dot :: rest2 ->
            let body, rest3 = parse_term rest2 in
            (TmTyAbs (param, bound, body), rest3)
        | _ -> raise (Parse_error "Expected '.' after type lambda parameter")
      end
