...
let rec eval env term =
  ...
  | TmTyAbs (param, body) -> VTyClosure (param, body, env)
  | TmTyApp (t, _) ->
      begin
        match eval env t with
        | VTyClosure (_, body, closure_env) -> eval closure_env body
        | _ -> raise (Runtime_error "Attempting to type-apply a non-polymorphic value")
      end
  | TmBool b -> VBool b
